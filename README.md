# Collab Tracker for Github Repositories
...
- Create a copy of ``collab/settings/local.py.example``:  
  `cp collab/settings/local.py.example collab/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/collab/settings/base.py``.
- Adjust ``backend/collab/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
